 ...
 *  in terms of java.awt.Shape. A ShapeRoi is constructed from a <code>ij.gui.Roi</code>
 *  object, or as a result of logical operators (i.e., union, intersection,
 *  exclusive or, and subtraction) provided by this class. These operators use
 *  the package <code>java.awt.geom</code> as a backend. <br>
...
public class ShapeRoi extends Roi {
	...
	/**
	 */
	final static int NO_TYPE = 128;

	/**
	 *  The maximum tolerance allowed in calculating the length of the curve
	 *  segments of this ROI's shape.
	 */
	final static double MAXERROR = 1.0e-3;

	/**
	 *  Coefficient used to obtain a flattened version of this ROI's shape. A
	 *  flattened shape is the closest approximation of the original shape's curve
	 *  segments with line segments.
	 */
	final static double FLATNESS = 0.1;

	/**
	 *  Parsing a shape composed of linear segments less than this value will
	 *  result in Roi objects of type {@link ij.gui.Roi#POLYLINE} and {@link
	 *  ij.gui.Roi#POLYGON} for open and closed shapes, respectively. Conversion of
	 *  shapes open and closed with more than MAXPOLY line segments will result,
	 *  respectively, in {@link ij.gui.Roi#FREELINE} and {@link ij.gui.Roi#FREEROI}
	 *  (or {@link ij.gui.Roi#TRACED_ROI} if {@link #forceTrace} flag is <strong>
	 *  <code>true</code></strong> .
	 */
	private final static int MAXPOLY = 10;// I hate arbitrary values !!!!

	private final static int OR = 0, AND = 1, XOR = 2, NOT = 3;

	private final static double SHAPE_TO_ROI = -1.0;
	...
	/**
	 *  The instance value of the maximum tolerance (MAXERROR) allowed in
	 *  calculating the length of the curve segments of this ROI's shape.
	 */
	private double maxerror = ShapeRoi.MAXERROR;

	/**
	 *  The instance value of the coefficient (FLATNESS) used to obtain a flattened
	 *  version of this ROI's shape.
	 */
	 ...
	 *  ROI's shape will work on a flattened version of the shape.
	 */
	private boolean flatten;

	/**
	 *  Flag which specifies how Roi objects will be constructed from closed
	 *  (sub)paths having more than <code>MAXPOLY</code> and composed exclusively
	 *  of line segments. If <strong><code>true</code></strong> then (sub)path will
	 *  be parsed into a {@link ij.gui.Roi#TRACED_ROI}; else, into a {@link
	 *  ij.gui.Roi#FREEROI}.
	 */
	private boolean forceTrace = false;

	/**
	 *  Flag which specifies if Roi objects constructed from open (sub)paths
	 *  composed of only two line segments will be of type {@link
	 *  ij.gui.Roi#ANGLE}. If <strong><code>true</code></strong> then (sub)path
	 *  will be parsed into a {@link ij.gui.Roi#ANGLE}; else, into a {@link
	 *  ij.gui.Roi#POLYLINE}.
	 */
	 ...
	 *@param  forceAngle  flag used in the conversion of Shape objects to Roi
	 *      objects (see {@link #shapeToRois()}.
	 ...
	 *      this ROI will be flattened (i.e., curve segments will be aproximated by
	 *      line segments).
	 *@param  maxPoly     Roi objects constructed from shapes composed of linear
	 *      segments fewer than this value will be of type {@link
	 *      ij.gui.Roi#POLYLINE} or {@link ij.gui.Roi#POLYGON}; conversion of
	 *      shapes with linear segments more than this value will result in Roi
	 *      objects of type {@link ij.gui.Roi#FREELINE} or {@link
	 *      ij.gui.Roi#FREEROI} (see {@link #shapeToRois()}).
	 */
	 ...
	 *  array.
	 *
	 *@param  shapeArray  Description of the Parameter
	 */
	...
	/**
	 *  Unary intersection operator. The caller is set to its intersection with the
	 *  argument (i.e., the overlapping regions between the operands).
	 *
	 *@param  sr  Description of the Parameter
	 *@return     the overlapping regions between <strong><code>this</code>
	 *      </strong> and <code>sr</code>
	 */
	...
	/**
	 *  Unary exclusive or operator. The caller is set to the non-overlapping
	 *  regions between the operands.
	 *
	 *@param  sr  Description of the Parameter
	 *@return     the union of the non-overlapping regions of <strong><code>this</code>
	 *      </strong> and <code>sr</code>
	 */
	...
	/**
	 *  Unary subtraction operator. The caller is set to the result of the
	...
	/**
	 *  Converts the Roi argument to an instance of java.awt.Shape. Currently, the
	 *  following conversions are supported:<br>
	 *
	 *  <table>
	 ...
	 *
	 *      <td>
	 *        closed java.awt.geom.GeneralPath
	 *      </td>
	 *
	 *      <td>
	 ...
	 *
	 *      <td>
	 *        open java.awt.geom.GeneralPath
	 *      </td>
	 *
	 *      <td>
	 ...
	 *
	 *@param  roi  Description of the Parameter
	 *@return      A java.awt.geom.* object that inherits from java.awt.Shape
	 *      interface.
	 */
	...
	/**
	 *  Gets the segment attribute of the ShapeRoi object
	 *
	 *@param  array  Description of the Parameter
	 *@param  seg    Description of the Parameter
	...
	/**
	 *  Converts a Shape into Roi object(s). <br>
	 *  This method parses the shape into (possibly more than one) Roi objects and
	 *  returns them in an array. <br>
	...
	/**
	 *  Gets the savedRois attribute of the ShapeRoi object
	 *
	 *@return    The savedRois value
	 */
	Roi[] getSavedRois() {
	...
	/**
	 *  Implements the rules of conversion from <code>java.awt.geom.GeneralPath</code>
	 *  to <code>ij.gui.Roi</code>.
	 *
	 *@param  segments    The number of segments that compose the path
	 *@param  linesOnly   Indicates wether the GeneralPath object is composed only
	 *      of SEG_LINETO segments
	 *@param  curvesOnly  Indicates wether the GeneralPath object is composed only
	 *      of SEG_CUBICTO and SEG_QUADTO segments
		...
		if (linesOnly) {
			switch (segments) {
							...
							case 2:
								...
								break;
							case 3:
								roiType = (closed ? Roi.POLYGON : (forceAngle ? Roi.ANGLE : Roi.POLYLINE));
								break;
							case 4:
								roiType = (closed ? Roi.RECTANGLE : Roi.POLYLINE);
								break;
							default:
								if (segments <= MAXPOLY) {
			...
			roiType = segments >= 2 ? Roi.COMPOSITE : NO_TYPE;
		}
		return roiType;
	}
	 ...
	 *@param  roiType  Description of the Parameter
	...
	/**
	 *  Checks whether the specified coordinates are inside a on this ROI's shape
	 *  boundaries.
	 *
	 *@param  x  Description of the Parameter
	 *@param  y  Description of the Parameter
	...
	/**
	 *  Returns the maximum Feret diameter (i.e., the largest distance between the
	 *  shape's boundaries). <br>
	 *  This method returns the maximum between the width or height of the bounding
	 *  rectangles of the rotating shape with one degree up to 270 degres, which
	 *  effectively means the maximum of its projections in R2.
	 *
	...
	/**
	 *  Returns the minimum Feret diameter (i.e., the smallest distance between the
	 *  shape's boundaries). <br>
	 *  This method returns the minimum between the width or height of the bounding
	 *  rectangles of the rotating shape with one degree up to 270 degres, which
	 *  effectively means the minimum of its projections in R2.
	 *
	 ...
	 *
	 *@param  s   Description of the Parameter
	 *@param  fl  Description of the Parameter
	 *@return     The flatteningPathIterator value
	 */
	...
	/**
	 *  Length of the control polygon of the quadratic B&eacute;zier curve
	...
	/**
	 *  Length of the chord of the arc of the cubic B&eacute;zier curve argument,
	 *  in double precision.
	 *
	 *@param  c  Description of the Parameter
	 *@return    Description of the Return Value
	...
	/**
	 *  Length of the chord of the arc of the quadratic B&eacute;zier curve
	 *  argument, in double precision.
	 *
	 *@param  c  Description of the Parameter
	 *@return    Description of the Return Value
	 ...
	 *  precision. The algorithm is based on the theory presented in paper <br>
	 *  &quot;Jens Gravesen. Adaptive subdivision and the length and energy of
	 *  B&eacute;zier curves. Computational Geometry <strong>8:</strong> <em>13-31
	 *  </em> (1997)&quot; implemented using <code>java.awt.geom.CubicCurve2D.Double</code>
	 *  . Please visit {@link <a href="http://www.graphicsgems.org/gems.html#gemsiv">
	 *  Graphics Gems IV</a> } for examples of other possible implementations in C
	 *  and C++.
	 *
	 *@param  c  Description of the Parameter
	 *@return    Description of the Return Value
	...
	/**
	 *  Calculates the length of a quadratic B&eacute;zier curve specified in
	 *  double precision. The algorithm is based on the theory presented in paper
	 *  <br>
	 *  &quot;Jens Gravesen. Adaptive subdivision and the length and energy of
	 *  B&eacute;zier curves. Computational Geometry <strong>8:</strong> <em>13-31
	 *  </em> (1997)&quot; implemented using <code>java.awt.geom.CubicCurve2D.Double</code>
	...
	/**
	 *  Scales a coordinate array with the size calibration of a 2D image. The
	 *  array is modified in place.
	 *
	 *@param  c   Array of coordinates in double precision with a <strong>fixed
	 *      </strong> structure:<br>
	...
	int k = c.length / 2;
		...
		if (2 * k != c.length) {
			return;
		}// bail out if array has odd length
	 ...
	 *  Description of the Method
	 *
	 *@param  pI  Description of the Parameter
	 *@return     Description of the Return Value
	 */
	Vector parseSegments(PathIterator pI) {
	Vector v = new Vector();

		if (parsePath(pI, null, v, null, null)) {
			return v;
		}
		return null;
	}
	 ...
	 *  The array contains a sequence of variable length segments that use from
	 *  from one to seven elements. The first element of a segment is the type as
	 *  defined in the PathIterator interface. SEG_MOVETO and SEG_LINETO segments
	 *  also include two coordinates, SEG_QUADTO segments include four coordinates
	 *  and SEG_CUBICTO segments include six coordinates.
	 *
	 *@return    The shapeAsArray value
	 */
		...
		for (int i = 0; i < s.size(); i++) {
			...
			switch (segType) {
							...
							case PathIterator.SEG_LINETO:
								...
								result[index++] = (float) p.getX() + x;
								result[index++] = (float) p.getY() + y;
								break;
							case PathIterator.SEG_QUADTO:
								result[index++] = segType;
								p = (Point2D.Double) h.elementAt(j++);
	 ...
	 */
	/**
	 *  Parses the geometry of this ROI's shape by means of the shape's
	 *  PathIterator and returns several convenience parameters in the arguments.
	 *  Iterates through the PathIterator argument and as a byproduct sets the
	 *  values of the other arguments passed to the method. To retrieve, for
	 *  example, the length (or perimeter) of a shape (or path), call this method
	 *  as<br>
	 *  <code> parsePath(pIter, par, null, null) <code>,
	 * <br>
	 *  where <code>par</code> is a double array of length one, then get the length
	 *  as <code>par[0]</code>
	 *
	 *@param  pIter     the PathIterator to be parsed.
	 *@param  params    an array with one elemet that will hold the calculated
	 *      length of path;
	 *@param  segments  a Vector of Integer objects that will hold the types of the
	 *      segments composing the shape's path
	 *@param  rois      a Vector that will hold ij.gui.Roi objects constructed from
	 *      elements of this path (see
	 *@param  handles   a Vector of Point2D.Double objects representing vertices
	 *      (segment joinings) and control points of the curves segments in the
	 *      iteration order;
	 *@return           <strong><code>true</code></strong> if successful.
	 *@link             #shapeToRois()} for details;
	 */
	...
	/**
	 *  Non-destructively draws the shape of this object on the associated
	 *  ImagePlus.
	 *
	 *@param  g  Description of the Parameter
	 */
	...
	/**
	 *  Draws the shape of this object onto the specified ImageProcessor. <br>
	 *  This method will always draw a flattened version of the actual shape (i.e.,
	 *  all curve segments will be approximated by line segments).
	 *
	...
	/**
	 *@return    <strong><code>false</code></strong> if type equals {@link
	 *      #NO_TYPE}.
	 */
	...
	/**
	 *  Sets the <code>java.awt.Shape</code> object encapsulated by <strong><code>this</code>
	 *  </strong> to the argument. <br>
	 *  This object will hold a (shallow) copy of the shape argument. If a deep
	 *  copy of the shape argumnt is required, then a clone of the argument should
	 *  be passed in; a possible example is <code>setShape(ShapeRoi.cloneShape(shape))</code>
	 *  .
	 *
	 *@param  rhs  The new shape value
	...
	/**
	 *  Adds a feature to the Circle attribute of the ShapeRoi class
	 *
	 *@param  sx      The feature to be added to the Circle attribute
	 *@param  sy      The feature to be added to the Circle attribute
	 *@param  swidth  The feature to be added to the Circle attribute
	 */
	 ...
	 *@param  swidth  Description of the Parameter
	 */
	...
	/**
	 *  Gets the circularRoi attribute of the ShapeRoi class
	 *
	 *@param  x      Description of the Parameter
	 *@param  y      Description of the Parameter
	 *@param  width  Description of the Parameter
	 *@return        The circularRoi value
	 */
