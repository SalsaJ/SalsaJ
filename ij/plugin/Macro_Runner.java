...
public class Macro_Runner implements PlugIn {
	 ...
	 *  file open dialog if <code>name</code> is an empty string. Loads the macro
	 *  from a JAR file in the plugins folder if <code>name</code> starts with
	 *  "JAR:". Otherwise, loads the specified macro from the plugins folder or
	 *  subfolder.
	 *
	...
	/**
	 *  Opens and runs the specified macro file on the current thread. The file is
	 *  assumed to be in the macros folder unless <code>name</code> is a full path.
	 *  ".txt" is added if <code>name</code> does not have an extension.
	 *
	 *@param  name  Description of the Parameter
	 ...
	 *  retrieve the optional string argument by calling the getArgument() macro
	 *  function. Returns the String value returned by the macro or null if the
