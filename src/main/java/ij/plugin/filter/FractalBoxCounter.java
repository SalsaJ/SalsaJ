 ...
 *  called, in fractal parlance, the "box counting" method. In the simplest
 *  terms, the routine counts the number of boxes of a given size needed to
 *  cover a one pixel wide, binary (black on white) border. The procedure is
 *  repeated for boxes that are 2 to 64 pixels wide. The output consists of two
 *  columns labeled "size" and "count". A plot is generated with the log of size
 *  on the x-axis and the log of count on the y-axis and the data is fitted with
 *  a straight line. The slope (S) of the line is the negative of the fractal
 *  dimension, i.e. D=-S. A full description of the technique can be found in T.
 *  G. Smith, Jr., G. D. Lange and W. B. Marks, Fractal Methods and Results in
 *  Cellular Morphology, which appeared in J. Neurosci. Methods, 69:1123-126,
 *  1996. --- 12/Jun/2006 G. Landini added "set is white" option, otherwise the
 *  plugin assumes that the object is always low-dimensional (i.e. the phase
 *  with the smallest number of pixels). Now it works fine for sets with D near
 *  to 2.0
 *
 *@author     Thomas
...
public class FractalBoxCounter implements PlugInFilter {
	...
	/**
	 *  Main processing method for the FractalBoxCounter object
	 *
	...
	PlotWindow plot = new PlotWindow("Plot", "log(box size)", "log(count)", px, py);
