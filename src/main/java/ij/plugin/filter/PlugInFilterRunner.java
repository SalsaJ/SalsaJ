...
public class PlugInFilterRunner implements Runnable, DialogListener {
	 ...
	 *@param  arg        The argument specified for this PlugInFilter in
	 *      IJ_Props.txt or in the plugins.config file of a .jar archive conatining
	 *      a collection of plugins. <code>arg</code> may be a string of length
	 *      zero.
	 */
	public PlugInFilterRunner(Object theFilter, String command, String arg) {
		...
		if ((flags & PlugInFilter.NO_IMAGE_REQUIRED) != 0) {
			imp = null;
		}// if the plugin does not want an image, it should not get one
	Roi roi = null;
		if (imp != null) {
			roi = imp.getRoi();
			if (roi != null) {
				roi.endPaste();
			}// prepare the image: finish previous paste operation (if any)
			if (!imp.lock()) {
				return;
		...
		if (theFilter instanceof ExtendedPlugInFilter) {// calling showDialog required?
			...
			}// ip has a snapshot that may be used for Undo
	 ...
	 *      May be null.
	 *@param  snapshotDone  Whether a snapshot of ip has been be taken previously.
	 *      if snapshotDone is true, the snapshot needed for the SUPPORTS_MASKING
	 *      or SNAPSHOT flags of the filter is not created any more. Class
	 *      variables used: flags (input), snapshotDone (set if a snapshot of ip is
	 *      taken), ipChanged (set if ip was probably changed).
	 */
	...
	boolean doMasking = (flags & PlugInFilter.SUPPORTS_MASKING) != 0 && ip.getMask() != null;
		...
		if (convertToFloat) {
			for (int i = 0; i < ip.getNChannels(); i++) {
				...
				if (thread.isInterrupted()) {
					return;
				}// interrupt processing for preview?
		...
		if (doMasking) {
			ip.reset(ip.getMask());
		}//restore image outside irregular roi
	}


	/**
	 *  test whether an ImagePlus can be processed based on the flags specified and
	...
	boolean imageRequired = (flags & PlugInFilter.NO_IMAGE_REQUIRED) == 0;
		...
		int type = imp.getType();
			...
			if ((flags & PlugInFilter.ROI_REQUIRED) != 0 && imp.getRoi() == null) {
				//EU_HOU Bundle
				IJ.error(cmd + ": " +IJ.getBundle().getString("SelReqErr"));
				return false;
			}
	...
	/**
	 *  Make the slice number accessible to the PlugInFilter by putting it into the
	...
	boolean previewDataOk = false;
		while (bgPreviewOn) {
			...
			long startTime = System.currentTimeMillis();
				...
				if (theFilter instanceof ExtendedPlugInFilter) {
					((ExtendedPlugInFilter) theFilter).setNPasses(nPasses);
				}//this should reset pass in the filter
				if (thread.isInterrupted()) {
					break interruptable;
				}
			...
			synchronized (this) {
				if (!bgPreviewOn) {
					break;
				}//thread should stop and possibly keep the data
				try {
		...
		if (thread.isInterrupted()) {
			previewDataOk = false;
		}//interrupted always means "don't keep preview"
	...
	/**
	 *  stop the background process responsible for preview as fast as possible and
	 *  wait until the preview thread has finished
	 */
     ...
     */
	/**
	 *  Sets the dialog attribute of the PlugInFilterRunner object
	 *
	 *@param  gd  The new dialog value
	 */
	...
	/**
	 *  The listener to any change in the dialog. It is used for preview. It is
	 *  invoked every time the user changes something in the dialog (except OK and
	 *  cancel buttons), provided that all previous listeners (parameter checking)
	 *  have returned true.
	 *
	 *@param  e   The event that has happened in the dialog. This method may be
	 *      also called with e=null, e.g. to start preview already when the dialog
	 *      appears.
	...
	public boolean dialogItemChanged(GenericDialog gd, AWTEvent e) {
		...
		if (previewThread != null) {//thread runs already
			...
			}//all other changes: restart calculating preview (with new parameters)
		}
		return true;
	}
}
