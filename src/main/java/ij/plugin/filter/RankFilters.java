 ...
 * Outliers and Despeckle commands.
 */
public class RankFilters implements ExtendedPlugInFilter, DialogListener {
    ...
    /**
     * Setup of the PlugInFilter. Returns the flags specifying the capabilities
     * and needs of the filter.
     *
     ...
     * @param kRadius The kernel radius. The kernel has a side length of
     * 2*kRadius+1
     * @param lineRadius The radius of the lines in the kernel. Line length of
     * line i is 2*lineRadius[i]+1. Note that the array
     * <code>lineRadius</code> will be modified, thus call this method with a
     * clone of the original lineRadius array if the array should be used again.
     ...
     * @param ignoreRight should be 0 for analyzing all data or 1 for leaving
     * out the row at the right
     * @param max should be -Float.MAX_VALUE or the smallest value the maximum
     * can be
     */
     ...
     * outside the kernel area. cache0 points to cache element equivalent to x
     */
    ...
    /**
     * Get sum of values and values squared within the kernel area. xCache0
     * points to cache element equivalent to current x coordinate. Output is
     * written to array sums[0] = sum; sums[1] = sum of squares
     */
     ...
     * left border outside the kernal area. Output is added or subtracted
     * to/from array sums[0] += sum; sums[1] += sum of squares when at the right
    ...
    /**
     * Get median of values and values squared within area. Kernel size kNPoints
     * should be odd.
     */
     ...
     * @param bufLength Number of values in
     * <code>buf</code> that should be read
     * @param n which value should be found; n=0 for the lowest, n=bufLength-1
     * for the highest
     * @return the value
     */
    ...
    /**
     * Create a circular kernel of a given radius. Radius = 0.5 includes the 4
     * neighbors of the pixel in the center, radius = 1 corresponds to a 3x3
     * kernel size. The output is written to class variables kNPoints (number of
     * points inside the kernel) and lineRadius, which is an array giving the
     * radius of each line. Line length is 2*lineRadius+1.
     */
