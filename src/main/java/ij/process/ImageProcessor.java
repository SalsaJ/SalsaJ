 ...
 *  types (byte, short, float and RGB) supported by ImageJ.
 *
 * @author     Thomas
...
public abstract class ImageProcessor extends Object {
     ...
     */
    public final static double NO_THRESHOLD = -808080.0;
    /**
     *  Left justify text.
     */
     ...
     */
    public final static int RED_LUT = 0, BLACK_AND_WHITE_LUT = 1, NO_LUT_UPDATE = 2, OVER_UNDER_LUT = 3;
    final static int INVERT = 0, FILL = 1, ADD = 2, MULT = 3, AND = 4, OR = 5,
            XOR = 6, GAMMA = 7, LOG = 8, MINIMUM = 9, MAXIMUM = 10, SQR = 11, SQRT = 12, EXP = 13, ABS = 14;
    final static int BLUR_MORE = 0, FIND_EDGES = 1, MEDIAN_FILTER = 2, MIN = 3, MAX = 4;
    final static String WRONG_LENGTH = "width*height!=pixels.length";
    int fgColor = 0;
    /**
     *  Description of the Field
     */
     ...
     *
     * @param  percentDone  Description of the Parameter
     */
    ...
    /**
     *  Returns this processor's color model. For non-RGB processors, this is the
     *  base lookup table (LUT), not the one that may have been modified by
     *  setMinAndMax() or setThreshold().
     *
    ...
    public ColorModel getColorModel() {
        ...
        if (baseCM != null) {
            //System.out.println("baseCM != null");
            return baseCM;
        } else {
    ...
    /**
     *  Returns the current color model, which may have been modified by
     *  setMinAndMax() or setThreshold().
     *
     * @return    The currentColorModel value
     */
    ...
    /**
     *  Sets the color model. Must be an IndexColorModel (aka LUT) for all
     *  processors except the ColorProcessor.
     *
     * @param  cm  The new colorModel value
     */
    ...
    /**
     *  Inverts the values in this image's LUT (indexed color model). Does nothing
     *  if this is a ColorProcessor.
     */
    ...
    /**
     *  Returns true if this image uses an inverting LUT that displays zero as
    ...
    /**
     *  Returns true if this image uses a pseudocolor or grayscale LUT, in other
     *  words, is this an image that can be filtered.
     *
     * @return    The pseudoColorLut value
     */
    ...
    /**
     *  Sets the default fill/draw value to the pixel value closest to the
     *  specified color.
     *
    ...
    /**
     *  This image will be displayed by mapping pixel values in the range min-max
     *  to screen values in the range 0-255. For byte images, this mapping is done
     *  by updating the LUT. For short and float images, it's done by generating
    ...
    /**
     *  For short and float images, recalculates the min and max image values
     *  needed to correctly display the image. For ByteProcessors, resets the LUT.
     */
    public void resetMinAndMax() {
    }

    /**
     *  Sets the lower and upper threshold levels. The 'lutUpdate' argument can be
     *  RED_LUT, BLACK_AND_WHITE_LUT, OVER_UNDER_LUT or NO_LUT_UPDATE. Thresholding
     *  of RGB images is not supported.
     *
    ...
    /**
     *  Returns the lower threshold level. Returns NO_THRESHOLD if thresholding is
     *  not enabled.
     *
     ...
     *
     * @return    The lutUpdateMode value
     */
     ...
     *  lutUpdateMode=NO_LUT_UPDATE. This removes the invisible threshold set by
     *  the MakeBinary and Convert to Mask commands.
     */
    ...
    /**
     *  Defines a rectangular region of interest and sets the mask to null if this
     *  ROI is not the same size as the previous one.
     *
     * @param  roi  The new roi value
     ...
     * @param  rwidth   The new roi value
     * @param  rheight  The new roi value
     * @see             ImageProcessor#resetRoi
     */
     ...
     *  to restore non-masked pixels. Here is an example: <pre>
     *ip.setRoi(new OvalRoi(50, 50, 100, 50));
     *ip.fill();
     *ip.reset(ip.getMask());
     *</pre> The example assumes <code>snapshot()</code> has been called, which is
     *  the case for code executed in the <code>run()</code> method of plugins that
     *  implement the <code>PlugInFilter</code> interface.
     *
     * @param  roi  The new roi value
    ...
    /**
     *  Defines a polygonal region of interest that will consist of a rectangular
     *  ROI and a mask. After processing, call <code>reset(mask)</code> to restore
     *  non-masked pixels. Here is an example: <pre>
    ...
    /**
     *  Sets the ROI (Region of Interest) and clipping rectangle to the entire
     *  image.
     */
    ...
    /**
     *  Defines a byte mask that limits processing to an irregular ROI. Background
     *  pixels in the mask have a value of zero.
     *
    ...
    /**
     *  Assigns a progress bar to this processor. Set 'pb' to null to disable the
     *  progress bar.
     *
     * @param  pb  The new progressBar value
     */
    ...
    /**
     *  Setting 'interpolate' true causes scale(), resize(), rotate() and getLine()
     *  to do bilinear interpolation.
     *
     * @param  interpolate  The new interpolate value
     */
    ...
    /**
     *  Returns an array containing the pixel values along the line starting at
     *  (x1,y1) and ending at (x2,y2). For byte and short images, returns
     *  calibrated values if a calibration table has been set using
     *  setCalibrationTable().
     *
    ...
    /**
     *  Inserts the pixels contained in 'data' into a horizontal line starting at
     *  (x,y).
     *
     * @param  x       Description of the Parameter
     * @param  y       Description of the Parameter
    ...
    public void drawOval(int x, int y, int width, int height) {
        if ((long) width * height > 4 * this.width * this.height) {
            return;
        }
     ...
     *
     * @param  xcenter  Description of the Parameter
     * @param  ycenter  Description of the Parameter
     */
    public void drawDot(int xcenter, int ycenter) {
        double r = lineWidth / 2.0;
        int xmin = (int) (xcenter - r + 0.5);
        int ymin = (int) (ycenter - r + 0.5);
        int xmax = xmin + lineWidth;
        int ymax = ymin + lineWidth;
        ...
        if (xmin < 0 || ymin < 0 || xmax >= width || ymax >= height) {
            ...
            double xoffset = xmin + r;
            double yoffset = ymin + r;
            double xx;
            double yy;
    ...
    /**
     *  Specifies whether or not text is drawn using antialiasing. Antialiased test
     *  requires Java 2 and an 8 bit or RGB image. Antialiasing does not work with
     *  8-bit images that are not using 0-255 display range.
     *
    ...
    /**
     *  Rotates the entire image 90 degrees clockwise. Returns a new ImageProcessor
    ...
    /**
     *  Rotates the entire image 90 degrees counter-clockwise. Returns a new
     *  ImageProcessor that represents the rotated image.
     *
     * @return    Description of the Return Value
     */
     ...
     *  Use fill(mask) to fill non-rectangular selections.
     *
    ...
    /**
     *  Fills pixels that are within the ROI and part of the mask (i.e. pixels that
     *  have a value=BLACK in the mask array). Throws and IllegalArgumentException
     *  if the mask is null or the size of the mask is not the same as the size of
     *  the ROI.
     *
    ...
    /**
     *  Set a lookup table used by getPixelValue(), getLine() and convertToFloat()
     *  to calibrate pixel values. The length of the table must be 256 for byte
     *  images and 65536 for short images. RGB and float processors do not do
     *  calibration.
     *
     * @param  cTable  The new calibrationTable value
     */
     ...
     *
     * @return    The calibrationTable value
     */
    /*
     ...
     */
    /**
     *  Gets the calibrationTable attribute of the ImageProcessor object
     *
     * @return    The calibrationTable value
     */
    public float[] getCalibrationTable() {
     ...
     *
     * @param  size  The new histogramSize value
     */
    ...
    /**
     *  Returns the number of float image histogram bins. The bin count is fixed at
     *  256 for the other three data types.
     *
     * @return    The histogramSize value
     */
    ...
    /**
     *  Set the range used for histograms of float images. The image range is used
     *  if both <code>histMin</code> and <code>histMax</code> are zero.
     *
     * @param  histMin  The new histogramRange value
     * @param  histMax  The new histogramRange value
     */
    ...
    /**
     *  Returns a reference to this image's pixel array. The array type (byte[],
     *  short[], float[] or int[]) varies depending on the image type.
     *
    ...
    /**
     *  Returns the value of the pixel at (x,y). For RGB images, the argb values
     *  are packed in an int. For float images, the the value must be converted
     *  using Float.intBitsToFloat(). Returns zero if either the x or y coodinate
     *  is out of range.
     *
     * @param  x  Description of the Parameter
     * @param  y  Description of the Parameter
    ...
    /**
     *  Gets the pixelCount attribute of the ImageProcessor object
     *
     * @return    The pixelCount value
     */
    ...
    /**
     *  This is a faster version of putPixel() that does not clip out of range
     *  values and does not do bounds checking.
     *
     * @param  x      Description of the Parameter
     * @param  y      Description of the Parameter
     * @param  value  Description of the Parameter
    ...
    /**
     *  Gets the intArray attribute of the ImageProcessor object
     *
    ...
    /**
     *  Sets the intArray attribute of the ImageProcessor object
     *
     * @param  a  The new intArray value
     */
    ...
    /**
     *  Gets the floatArray attribute of the ImageProcessor object
     *
     * @return    The floatArray value
     */
    ...
    /**
     *  Sets the floatArray attribute of the ImageProcessor object
     *
     * @param  a  The new floatArray value
     */
    ...
    /**
     *  Returns the samples for the pixel at (x,y) in an int array. RGB pixels have
     *  three samples, all others have one. Returns zeros if the the coordinates
     *  are not in bounds. iArray is an optional preallocated array.
     *
     * @param  x       Description of the Parameter
     * @param  y       Description of the Parameter
    ...
    /**
     *  Sets a pixel in the image using an int array of samples. RGB pixels have
     *  three samples, all others have one.
     *
     * @param  x       Description of the Parameter
     * @param  y       Description of the Parameter
     * @param  iArray  Description of the Parameter
    ...
    /**
     *  Uses bilinear interpolation to find the pixel value at real coordinates
     *  (x,y). Returns zero if the (x, y) is not inside the image.
     *
     * @param  x  Description of the Parameter
     * @param  y  Description of the Parameter
     * @return    The interpolatedValue value
     */
    ...
    /**
     *  Gets the interpolatedEdgeValue attribute of the ImageProcessor object
     *
     * @param  x  Description of the Parameter
     * @param  y  Description of the Parameter
    ...
    /**
     *  Gets the edgeValue attribute of the ImageProcessor object
     *
     * @param  x  Description of the Parameter
     * @param  y  Description of the Parameter
     * @return    The edgeValue value
     */
     ...
     *  image boundary. For 8-bit and 16-bit images, out of range values are
     *  clipped. For RGB images, the argb values are packed in 'value'. For float
     *  images, 'value' is expected to be a float converted to an int using
    ...
    /**
     *  Returns the value of the pixel at (x,y). For byte and short images, returns
     *  a calibrated value if a calibration table has been set using
     ...
     *  to width*height. Use setSnapshotPixels(null) to clear the snapshot buffer.
     *
    ...
    /**
     *  Transforms the image or ROI using a lookup table. The length of the table
     *  must be 256 for byte images and 65536 for short images. RGB and float
    ...
    /**
     *  If this is a 32-bit or signed 16-bit image, performs an absolute value
     *  transform, otherwise does nothing.
     */
    public void abs() {
    }
    ...
    /**
     *  Makes a copy of this image's pixel data that can be later restored using
     *  reset() or reset(mask).
     *
    ...
    /**
     *  Restores pixels from the snapshot buffer that are within the rectangular
     *  roi but not part of the mask.
     *
     * @param  mask  Description of the Parameter
     */
    ...
    /**
     *  Convolves the image or ROI with the specified 3x3 integer convolution
     *  kernel.
     *
    ...
    /**
     *  A 3x3 filter operation, where the argument (BLUR_MORE, FIND_EDGES, etc.)
    ...
    /**
     *  Creates a new processor containing an image that corresponds to the current
     *  ROI.
     *
     * @return    Description of the Return Value
     */
     ...
     *  Only works with 8-bit and 16-bit images.
     *
     ...
     */
    /**
     *  Translates the image or selection tx and ty.
     *
     ...
     */
    /**
     *  Returns the histogram of the image or ROI. Returns a luminosity histogram
     *  for RGB images and null for float images.
     *
    ...
    /**
     *  For 16 and 32 bit processors, set 'lutAnimation' true to have createImage()
     *  use the cached 8-bit version of the image.
     *
     * @param  lutAnimation  The new lutAnimation value
     */
     ...
     *
     * @param  doScaling  Description of the Parameter
     * @return            Description of the Return Value
     */
    ...
    /**
     *  Returns a 32-bit float version of this image as a FloatProcessor. For byte
     *  and short images, converts using a calibration function if a calibration
    ...
    /**
     *  Performs a convolution operation using the specified kernel. KernelWidth
     *  and kernelHeight must be odd.
     *
     * @param  kernel        Description of the Parameter
     ...
     *  determined threshold. For RGB images, converts each channel to binary. For
     ...
     *  objects and background. It does this by taking a test threshold and
     *  computing the average of the pixels at or below the threshold and pixels
     *  above. It then computes the average of those two, increments the threshold,
     *  and repeats the process. Incrementing stops when the threshold is larger
     *  than the composite average. That is, threshold = (average background +
     *  average objects)/2. This description was posted to the ImageJ mailing list
     *  by Jordan Bevic.
     *
     * @return    The autoThreshold value
     */
     ...
     *  argument.
     *
     * @param  histogram  Description of the Parameter
     * @return            The autoThreshold value
     */
     ...
     *  drawPixel(). The clipping rectangle is reset by passing a null argument or
     *  by calling resetRoi().
     *
     * @param  clipRect  The new clipRect value
     */
    ...
    /**
     *  Gets the indexSampleModel attribute of the ImageProcessor object
     *
     * @return    The indexSampleModel value
     */
     ...
     *
     * @return    The defaultColorModel value
     */
    ...
    /**
     *  The getPixelsCopy() method returns a reference to the snapshot buffer if it
     *  is not null and 'snapshotCopyMode' is true.
     *
     * @param  b  The new snapshotCopyMode value
    ...
    /**
     *  Returns the number of color channels in the image. The color channels can
     *  be accessed by toFloat(channelNumber, fp) and written by
    ...
    public int getNChannels() {
        return 1;
        /*
         *  superseded by ColorProcessor
         */
    }

    /**
     *  Returns a FloatProcessor with the image or one color channel thereof. The
     *  roi and mask are also set for the FloatProcessor.
     *
     ...
     *      FloatProcessor is overwritten when converting data (re-using its pixels
     *      array improves performance).
     * @return                A FloatProcessor with the converted image data of the
     *      color channel selected
     */
    public abstract FloatProcessor toFloat(int channelNumber, FloatProcessor fp);

    /**
     *  Sets the pixels (of one color channel for RGB images) from a
     *  FloatProcessor.
     *
